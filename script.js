...
function updateInfo() {
  ...
  $("trackArtist").textContent =
    `${d.author || "YouTube"}${list.length && i >= 0 ? ` • Track ${i + 1} of ${list.length}` : ""}`;
}
...
function skipToNextTrack() {
  ...
  // Restart only after the last track
  if (list.length > 0 && nextIndex >= list.length) {
    nextIndex = 0;
  }

  setStatus(`Skipping to Track ${nextIndex + 1}...`);
...
function onYouTubeIframeAPIReady() {
  player = new YT.Player("player", {
    ...
    events: {
      ...
      // If YouTube gives Error 150 or another video error,
